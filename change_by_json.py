...
REPLACEMENTS = dict()
with open(bj('ren-rs.config.json'), 'r', encoding='UTF-8') as f:
    ...
    for key in args:
        if key in PATTERN_MAP:
            ...
            print(f'未知 JSON 参数：{key}', file=sys.stderr)
            print(f'支持的 JSON 参数：{list(PATTERN_MAP.keys())}', file=sys.stderr)
            sys.exit(1)
...
RAW_UNDERLINE_IDENTIFIER = '_'.join(PATTERN_MAP['--project_identifier'].split('.')[:-1])
...
NEW_UNDERLINE_IDENTIFIER = '_'.join(REPLACEMENTS[PATTERN_MAP['--project_identifier']].split('.')[:-1])
